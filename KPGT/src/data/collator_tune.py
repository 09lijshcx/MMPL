...
class Collator_pretrain(object):
    ...
    def __call__(self, samples):
        smiles_list, fps, mds, label, logd, logp, pka, pkb, logsol, wlogsol = map(list, zip(*samples))
        graphs = []
        ...
        logd = torch.tensor(logd, dtype=torch.float32)
        logp = torch.tensor(logp, dtype=torch.float32)
        pka = torch.tensor(pka, dtype=torch.float32)
        pkb = torch.tensor(pkb, dtype=torch.float32)
        logsol = torch.tensor(logsol, dtype=torch.float32)
        wlogsol = torch.tensor(wlogsol, dtype=torch.float32)
...
class Collator_tune(object):
    ...
    def __call__(self, samples):
        smiles_list, fps, mds, label = map(list, zip(*samples))
        graphs = []
        for smiles in smiles_list:
            graphs.append(smiles_to_graph(smiles, self.vocab, max_length=self.max_length, n_virtual_nodes=self.n_virtual_nodes, add_self_loop=self.add_self_loop))
        batched_graph = dgl.batch(graphs)
        ...
        batched_graph.edata['path'][:, :] = preprocess_batch_light(batched_graph.batch_num_nodes(), batched_graph.batch_num_edges(), batched_graph.edata['path'][:, :])
        label = torch.tensor(label, dtype=torch.float32)
        return smiles_list, batched_graph, fps, mds, label
