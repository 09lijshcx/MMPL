...
class Collator_pretrain(object):
    ...
    def __call__(self, samples):
        smiles_list, fps, mds, text = map(list, zip(*samples))
        graphs = []
        ...
        # text = torch.stack(text, dim=0).reshape(len(smiles_list),-1)
        return smiles_list, batched_graph, fps, mds, sl_labels, disturbed_fps, disturbed_mds, text
