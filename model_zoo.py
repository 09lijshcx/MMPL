...
class Pretrain_SCIBert_processor(nn.Module):
    ...
    def forward(self, batch_sentences):
        ...
        all_tokens, _ = text_features # AutoModel
...
class CLIPM_KPGT(nn.Module):
    def __init__(self, kpgt, model):
        super().__init__()
...
class Molecule_Prompts_Interatctoin_Module(nn.Module):
    def __init__(self, num_layer=1, n_cls=1):
        ...
        # self.positional_embedding = nn.Parameter(torch.randn(self.attr_num+1+n_cls+32, 768))  #known property + "unknown" + [cls] from text enc. + mol tokens
        self.positional_embedding = nn.Parameter(torch.randn(n_cls+32, 768))  #known property + "unknown" + [cls] from text enc. + mol tokens
        # nn.init.normal_(self.positional_embedding, std=0.01)
        
        self.moltex_attn = Moltex_Attention(768, n_cls)
    
    def forward(self, mol_feats, prompts, fd, ind=None):
        prompts = prompts.unsqueeze(1).repeat(1, mol_feats.shape[1], 1)
